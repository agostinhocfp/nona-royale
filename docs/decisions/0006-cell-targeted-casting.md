...
- **Closed 2026-09-15: beacons were not drawn.** When this record was written, `GameEngine.ActiveBeacons()` had no caller in the Unity assembly, so a beacon was visible only in the event log, which is the trap this record rejects. `GameEngine.ActiveCellEffects()` now returns each pending effect with its owner and the cells it covers (computed by `TargetingRules.CellsInArea`), and `DeviceLayer` draws them after every batch of events.
- **Open: `ClearFor(owner)` has no caller yet.** Its remarks reserve it for a match reset or a future effect that disarms devices. Whether a seat's devices should also be cleared in other cases (for example, when its last operator gets home) is undecided.
