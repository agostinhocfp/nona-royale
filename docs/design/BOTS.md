...
- 2026-09-24 — **Lethe's rework taught** (`COMBAT_SYSTEMS.md` §10.10). `CastPlanner` counts as caught every enemy Eris' Exploit's draw can bring inside its zone (reach = zone radius + draw cells, capped at the draw's reach), and gives the drag itself no value. **Measured and rejected:** scoring the drag as lost enemy ground (full `Progress` weight, and 0.3 of it). At full weight the bots cast at lone pieces — 41% of casts struck nobody — and Lethe fell to 24.5%; at 0.3, 25.5%; unscored, 26.5%. A setback to one opponent is shared by the other two. Nano Cell's stun term simply stopped applying. `LetheBotTests`: two new (the drawable crowd, the lone drawable enemy), the stun test rewritten. **Not taught:** following Lethe's wake, and scattering from her reach.
- 2026-09-24 — **Evasion is priced only when it can fire** (`COMBAT_SYSTEMS.md` §5.5, §10.3). `BotBoard.ExpectedHitOnceExposed` discounted every Normal hit on an Evasion holder by the full 12%, even after the round's one roll was spent, and ignored Tech, which the pipeline also rolls against. Measured on the pre-removal Kurbyn: that alone steered bots off him for about 1.4 of his 2.5 evasion points (28.9% → 27.5% with the discount gone). It now asks `GameEngine.EvasionReady` and covers Normal and Tech. Dormant: nobody holds Evasion. Blind Spot's new slow needed no planner change; `StatusWorth(Slow)` already prices it, and casts rose 1.18 → 1.71 a match.
