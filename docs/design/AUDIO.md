...
| AU4 | **Match playlist** | Five full Lyria takes as `Match`, `Match_2` … `Match_5` (OGG), streamed; `MusicPlaylist` (shuffle bag, no repeats back to back); `SoundBank.MusicTracks`; `AudioDirector.StartTrack` plays each take through, then the next after a breath. |
...
- 2026-09-21 — **AU4 follow-up: the match tracks ship as OGG, numbered 1 to 5.** The designer renumbered the raw takes in `Claude outputs/lyria/` (the Showdown duplicate removed; the title take is now `Match_4`, Sicilian Waltz `Match_5`) and exported OGGs there, to cut the LFS weight of about 145 MB of WAVs.
  - **Flagged:** those OGGs were encoded from the raw takes at about 100 kb/s, so they skipped the AU4 processing (48 kHz, −12.9 to −17.1 LUFS, and two peaking above 0 dBFS, so they clip). They were left in `Claude outputs/lyria/` untouched, and not used.
  - **In the game instead:** the processed AU4 masters encoded with libvorbis q7 (44.1 kHz, about 190–210 kb/s, still −16 LUFS, peaks −1.3 to −3.1 dBFS): `Match.ogg`, `Match_2.ogg` … `Match_5.ogg` in `Audio/Resources/Audio/Music/`, 20 MB in all against 145 MB. Unity re-encodes music to Vorbis on import anyway, so q7 costs nothing audible.
