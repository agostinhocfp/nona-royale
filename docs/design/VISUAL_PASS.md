...
- 2026-09-20 — **Pass closed. Play Mode passed on all five outstanding changes.**
  ...
  - **What the pass left behind, for whoever picks the board up next:**
    - ~~`seat_base` and `seat_ring` ride the hop and fly with the figure.~~ **Fixed 2026-09-20**, after V2's contact shadow had proved the mechanism: `Contact` became `GroundMarks` and now un-hops all three. The bug predates the tilt - under the flat camera the lift is straight up the screen, so a disc sliding up behind a piece read as the piece's own shadow moving, and only the tilt turned it into a mark flying off the table.
