...
- **Four events:** `DebtIncurred` (the cast), `DebtCollected` (the bill, emitted before `TurnEnded`), `DebtCalled` (Sadist), `DebtBurned` (the collision). `EnergyDrained` is retired. In the history strip the lines join the hover card of the action they belong to; a turn closed with nothing else in its segment gets one quiet `DEBT` chip, no toast. **No other view change was made**: a seat readout for the figure is deferred to the UI pass.
...
| 3   | **Sadist**      | Active (Ult) | 7    | 3   | 3     | **The target's seat debt** as Normal, **at least 2** (0 → 2, 4 → 4, 6 → 6); **half that** to enemies within 2 of the target; the debt is cleared (§3.3). |
...
**Mirror match:** Sadist is a 7-cost cast, so Equilibrium still halves it against another Revú, to 3 at most and 1 at the floor. Leech Round is cheap, so its 2 lands as 4.
