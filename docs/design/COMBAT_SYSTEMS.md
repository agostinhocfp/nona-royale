...
- **What moved with it:** Luka's heavy line went 6 → 7 (§2.4), so "heavy" still means a short list and not most of the roster — the Bouncer, Sanity and, as this doc failed to say until 2026-09-23, Revú, whose 8 clears the line too.
...
- **Heavy means maximum health above a threshold the effect states** (Luka's is 7, raised from 6 with the roster-wide +1 health on 2026-09-16: today the Bouncer at 10, Sanity at 9 and Revú at 8). Maximum, not current — heavy is who an operator is, not how hurt it is. Revú crossed the line with that +1 and this doc kept calling it "the two tanks" until 2026-09-23; the code was always right, because it reads `MaxHealth` and not a list of names.
- **A damage effect may also carry a flat heavy bonus** (`AbilityEffect.WithHeavyBonus`, 2026-09-23), added to the amount **before** any multiplier, in the same place as the bleeding bonus. It reads the same heavy line the crit multiplier reads, so one effect can carry both and neither needs the other. Luka's Blind Spot is the first user (§10.9).
...
### 3.3 Debt, and debt-scaled damage

_(Added 2026-09-17 with Revú as a drain and pool-scaled damage; reworked into debt 2026-09-24, designer.)_

- **A seat can owe.** Debt is **one figure per seat**, held beside its pool — never on a piece (designer). Nothing about it is drawn on the board; the match history carries it (below). `PlayerState.Debt`, changed only by `EnergyLedger`.
- **Incurring it takes nothing.** `EffectKind.IncurDebt` adds to the target's seat, up to **`EnergyConfig.DebtCap` = 6**, and reports what it actually added, even 0 at the cap. The caster is recorded as a **creditor**, even when the cap refuses the amount. Leech Round adds 2 (§10.11).
- **Never paid; it grows** (designer, 2026-09-24, after the measurement in §10.11). Each time the debtor ends its own turn, before its statuses expire, the whole debt draws **`DebtInterest` = 1**, never past the cap. No energy moves. A debt ends only when Sadist calls it or a collision burns it. `EnergyLedger.AccrueDebt`, reported as `DebtAccrued` — and only when it actually grew, so a debt sitting at the cap says nothing every turn.
  - **Why at the end of the debtor's turn and not at its upkeep:** a loan made on an opponent's turn draws its first interest only after the debtor has had a whole turn of its own to answer it — by landing on its creditor.
  - **The first version collected** (morning of 2026-09-24): the pool paid what it could, destroyed, and only an unpaid remainder drew interest. A seat almost always could pay, so the debt was the old drain one turn late and never built (§10.11). A **voluntary pay-off** — a seat spending its own energy to clear its debt — is the designer's open option on top of this rule, not built.
- **Debt-scaled damage reads the target's seat at cast time.** `EffectKind.DebtDamage` deals the seat's whole debt, **at least the effect's minimum** (Sadist: 2), to the target, and that figure halved, rounded down, to enemies within 2 of it, whatever their own seats owe. A share of 0 is not dealt at all. Then **the debt is cleared, whatever the hits did** — evaded, shielded or halved by Equilibrium (§5.17), the collection has happened; a dodge is not a second life for the loan.
- **The floor** (2026-09-21, kept through the rework). It is on the primary figure only and the splash divides the floored figure, so a seat that owes nothing still takes 2 and its neighbours 1. It is a floor on what the cast **computes**: Equilibrium and every mitigation layer still apply after it (§2.1).
- **Burning it.** When one of the debtor seat's operators collides with any of its creditors — a dice move landing on him, whoever wins the contest (§7) — the whole debt burns. Placement never collides, so a swap, pull or push never burns anything.
- **Both effects need the seats.** `AbilityResolver` takes the match's players at composition. A resolver built without them refuses to run either effect instead of guessing.
- **Four events:** `DebtIncurred` (the cast; carries the operator it was aimed at), `DebtAccrued` (the interest, emitted before `TurnEnded`), `DebtCalled` (Sadist; carries its target), `DebtBurned` (the collision). `EnergyDrained` is retired. In the history strip the lines join the hover card of the action they belong to; a turn closed with nothing else in its segment gets one quiet `DEBT` chip, no toast.
- **How the view writes it (2026-09-24, designer): Roman numerals on the display face, in oxblood and brass** (`DebtMark`), so a debt can never be read as health or energy, which are Arabic on the data face. **The standing figure is the seat's, so it lives beside the seat's pool and never on a piece:** a blood-velvet chip in every form of the squad rail (header, folded spine, upright band) and in the top bar for the seat to play, hidden at zero, popping when the figure changes. **The board shows debt only while something happens to it:** "+II" rises off the operator a loan was aimed at, Sadist stamps the figure it called over its target a size above any damage number with a blood-velvet ring, and a burn strikes the figure through in ember over the creditor. Interest has no piece to land on, so it shows only as the chip changing.
- **What it replaced, and why.** The 2026-09-17 drain took 2 on the spot, which gave the victim no decision and a spectator nothing to follow, and Sadist read `floor((cap − pool) ÷ 3)`, a figure nobody computes at the table. The debt makes the ultimate's figure the one number the debtor has been looking at, and — since it stopped being collected — a number that grows while it is ignored.
- **Transfers are still Ghost's**, if he is built.
...
- **The web it builds:** cheap casts are the best way to kill him, but every cast is energy not kept back for the debt his Leech Round runs up, and unpaid debt is what Sadist collects. The honest answer is collisions: dice combat costs no energy, bypasses Equilibrium and burns the debt (§3.3).
...
- **Heavy targets take a bonus** (Blind Spot: +1 on top of 1 when maximum health is above 7). Settled at cast time — maximum health never changes. Since 2026-09-23 the instant strike carries the same rider independently (§2.4), so a heavy target pays it twice if the follow-up lands and once if it escapes.
...
- **How much was the bots?** With evasion kept but the bots no longer discounting hits on him, he measured **27.5%** (deaths 0.87). About 1.4 of the 2.5 points was the bots pricing every Normal hit on him at 88%, even after the round's one roll was spent, and aiming elsewhere; about 1.1 was the dodges themselves. The pricing is fixed (`BotBoard` asks `GameEngine.EvasionReady`, Normal and Tech, as the pipeline has it), dormant with the rule.
...
| 3   | **Killzone**         | Active | 9    | 4   | 2 (cell) | Deploy a zone on a cell (ADR-0007). At Nuetu's next upkeep it detonates: **1 Normal** and **Stun 2 turns** to every enemy within **2** (1 until 2026-09-24). Then **1 Normal** to each enemy standing in it at his next **two** upkeeps. Nuetu **heals 1** on cast. |
...
| #   | Ability          | Type         | Cost | CD  | Range | Effect                                                                                                                                                                                                                              |
| --- | ---------------- | ------------ | ---- | --- | ----- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **Blind Spot**   | Active       | 5    | 3   | 3     | **Teleport** to the enemy (§7.6 landing, no path damage); **2 Normal**, **3** if its max HP is above 7, and **Slow 1 turn**. **Follow-up** (§6.5): at Luka's next upkeep, if he is within **2** of the target, **1 Normal** — **2** if its max HP is above 7. Hunted marker; cleansable. |
...
**"Heavy" is maximum health above 7** (6 until the roster-wide +1 of 2026-09-16) — today the Bouncer at 10, Sanity at 9 and Revú at 8. All three of his damage riders read it.

**Blind Spot's strike pays the heavy bonus too (designer, 2026-09-23).** It used to be the follow-up's alone: the teleport hit everything for 2 and only the delayed blow knew the difference between a tank and everyone else. Now both halves read the same line, so a heavy target takes **3 on the cast and 2 on the follow-up** — 5 if it stays inside 2 cells, 3 if it runs, against 3 and 2 for anyone else. The rider is paid whether or not the follow-up resolves, which is the point: the certain half of the cast is the half that should care who it hit. **Bots sweep, 1600 matches, paired seeds: Luka 26% → 27%, Sanity 25% → 24%, Bouncer and Revú flat, Blind Spot 1.11 casts a match either way — all of it inside noise, and it should be.** Three of twelve operators are heavy and the cast lands about once a match, so the change is worth roughly a quarter of a point of damage per match at bot scale. It is a consistency change, not a lever. **Watch item:** it is a small nick at Revú, who is already bottom-third (§10.11).

**Blind Spot's strike slows for one round (designer, 2026-09-24).** A one-turn Slow (§5.2) on the target, applied on Luka's turn, so it is live for exactly the target's next turn: the one it spends getting clear of the follow-up's reach of 2. At half speed a roll still moves it, just not as far, so the escape is priced, not removed; a cleanse takes the slow with the marker. `Luka.StrikeSlowTurns`.

| Bots against bots, 4,000 paired matches | Before | After |
| --------------------------------------- | ------ | ----- |
| Luka win share                          | 25.5%  | 24.8% |
| Blind Spot casts per match              | 1.18   | 1.71  |
| Follow-ups that land                    | 17%    | 27%   |
| Vendetta casts per match                | 2.22   | 1.91  |

- **More Blind Spot, and more of it connects.** The bots value the slow, so they cast it half again as often, and the follow-up lands about one time in four instead of one in six.
- **Win share unchanged within noise** (−0.7, at the edge of ±0.7). The extra casts come partly out of Vendetta (2.22 → 1.91), his better finisher, so a human Luka may do better with it than the bots do. Watch it; the dial is `StrikeSlowTurns`, or the slow itself.
...
| #   | Ability         | Type         | Cost | CD  | Range | Effect                                                                                                                                           |
| --- | --------------- | ------------ | ---- | --- | ----- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | **Leech Round** | Active       | 3    | 1   | 3     | **2 Normal**; the target's seat takes on **2 debt** (§3.3), which grows by 1 each turn it ends owing.                                              |
| 2   | **Equilibrium** | Passive      | —    | —   | self  | A cast's instant damage to him is **×2** at cost ≤ 3, **½ (at least 1)** at cost ≥ 6, unchanged at 4–5 (§5.17).                                   |
| 3   | **Sadist**      | Active (Ult) | 6    | 3   | 3     | **The target's seat debt** as Normal, **at least 2** (0 → 2, 4 → 4, 6 → 6); **half that** to enemies within 2 of the target; the debt is cleared (§3.3). |
...
- Leech Round's energy is **destroyed**, not transferred — kept through the 2026-09-24 rework: paid debt is destroyed too.
...
**Readability, as the draft demanded:** since 2026-09-24 Sadist's figure is the target seat's debt — the one number its owner has been looking at since the loan was made. The 2026-09-17 figure, "one damage for every 3 energy missing", was computable but nobody computed it.

**Mirror match:** Sadist is a 6-cost cast (7 until 2026-09-24), still in the dear band, so Equilibrium halves it against another Revú, to 3 at most and 1 at the floor. Leech Round is cheap, so its 2 lands as 4.
...
**Reworked 2026-09-24 (designer): the drain becomes debt** (§3.3). Leech Round no longer takes energy; it puts the target's seat 2 in debt, which in this first version was paid from its pool as its turn ended, with an unpaid remainder drawing 1 interest up to a cap of 6 (superseded the same day: see below). Sadist deals the target seat's debt (at least 2) and clears it. A collision against Revú by the debtor's side burns the whole debt. The debt lives on the seat, not on a piece, so the board carries no new marker (designer). **Numbers unchanged:** 3 / 1 / 3 and 7 / 3 / 3, health 8.

| Bots against bots, 800 matches, same seeds | Drain (HEAD `b8a01a7`) | Debt |
| ------------------------------------------ | ---------------------- | ---- |
| Revú win share                             | 25%                    | 27%  |
| Leech Round casts per match                | 2.43                   | 2.56 |
| Sadist casts per match                     | 1.18                   | 0.77 |
| Collisions per match                       | 3.20                   | 3.08 |
| Turns per seat                             | 26.1                   | 25.7 |
| Knockouts per match                        | 12.5                   | 12.0 |

- **Revú +2 points, inside the ±1.6 noise of 800 matches** but at the top of the table (level with Kurbyn, Bouncer and Javi). Mimi 22% → 20% is the only other move of two points, and she has no contact with the change. Re-measure at 4000 paired matches before tuning.
- **The debt ledger across the sweep's 3,200 matches (all four rows):** 11,423 debt incurred, **8,976 paid (79%)**, 18% of collections left some debt carried, and 908 points of interest. **Sadist called in 0 debt 35% of the time, 2 57%, 3 or more 8%.** A bot pays a debt of 2 almost every time, so for the bots the debt is mostly the old drain one turn late, and Sadist mostly lands its floor.
- **What the harness cannot tell us:** how often a human carries a debt on purpose to fire a big cast, and whether Sadist is then worth waiting for. The bots cast Sadist into a seat that owes nothing a third of the time; a human would wait. That is the first thing to watch at the table.
- **Burns: 323 across the 3,200 matches**, about one in ten matches. Collisions did not rise; the burn is priced but rarely on offer, because the debtor has to be on the dice line behind him.
- **Dials, if the debt reads as a formality:** Leech Round's debt 2 → 3 (a squeezed seat carries more often), interest charged on the whole debt rather than the remainder, or a lower cap with a higher Sadist floor. Each is a constant in `Revu.cs` or `EnergyConfig`.

**Why the debt does not bite — measured 2026-09-24** (2,000 bots-against-bots matches per row, paired seeds, on HEAD `d03bb69`: Sadist at 6, Killzone radius 2; ±1 point). A throwaway harness logged every debt event; none of the variants below is in the code.

| Variant | Revú | Sadist casts | Paid | Mean seat debt | Sadist called 3+ |
| --- | --- | --- | --- | --- | --- |
| **As built** (auto-collect, interest on the remainder) | 25.8% | 0.96 | 74% | 0.03 | 2% |
| Leech Round debt 2 → 3 | 26.0% | 1.06 | 69% | 0.05 | 81% (all 3s) |
| Interest on the whole debt | 24.6% | 0.97 | 112% | 0.03 | 4% |
| Debtors ignore their bill (bots) | 25.6% | 0.96 | 76% | 0.03 | 4% |
| Collect an installment of 1 a turn | 25.9% | 0.93 | 64% | 0.04 | 3% |
| Collect every other turn | 25.4% | 0.98 | 67% | 0.05 | 5% |
| Paid debt goes to Revú's seat | 26.2% | 1.04 | 72% | 0.03 | 2% |
| Leech Round range 3 → 6 | 26.6% | 0.99 | 88% | 0.06 | 6% |
| Hitting Revú with a cast owes 1 | 26.1% | 0.99 | — | 0.04 | 7% |
| Bots cast Sadist only on debt 3+ | 25.9% | **0.05** | 97% | 0.04 | 100% |
| **No automatic collection** (debt only grows, +1 a turn to 6; cleared by Sadist or a burn) | 26.7% | 1.24 | 0% | **0.68** | **53%** (31% at 6) |
| The same, bots cast Sadist only on debt 4+ | 26.8% | 0.73 | 0% | 0.87 | 100% (66% at 6) |

- **The debt never builds while it is collected every turn.** Revú lends at most once a turn, to one seat, about 2.6 times a match; the debtor pays at each of its own turn ends, and before paying it holds 2 or more 84% of the time. Every collection rule tested — installments, every other turn, interest on the whole — leaves the mean seat debt under 0.1.
- **So Sadist is a floor, not a collection.** Two thirds of its casts call in exactly 2, and that 2 is Leech Round cast in the same turn, not a debt anybody carried. A bot that waits for 3 or more casts it 0.05 times a match: at Revú's turn, a debt of 3 does not exist.
- **Leech Round 3 is not the fix it was billed as.** It moves the figure from 2 to 3 because the same-turn combo now computes 3; nothing accumulates.
- **Stopping the automatic collection is the one change that makes the debt a number that grows,** and it costs nothing in balance: Revú 26.7% against 25.8%, inside noise. The mean debt on the table goes from 0.03 to 0.68, Sadist collects 4 to 6 in 44% of its casts, and a burn is worth 3.8 on average instead of 2. The trade is Leech Round's energy denial, which disappears unless the debtor can pay.
- **The honest shape of the fix**, for the designer: stop the automatic collection, and let a seat **pay its debt off on purpose** — a tap on its own debt chip, from its pool, at any point in its turn. That is the pay-or-spend decision the rework promised, made explicit, and the numerals already give it a place to press. It needs a command, a bot policy for when to pay, and a sweep of that policy; none of it is built.

**Adopted 2026-09-24 (designer): no automatic collection, and no pay-off yet.** The debt grows by 1 each turn the debtor ends owing, to 6, and ends only in Sadist or a burn (§3.3). The pay-off stays open, to be judged after human games. Measured on the same harness, seeds and HEAD as the table above: **Revú 26.6%** (25.8% as built), Sadist 1.31 casts a match (0.96), mean seat debt 0.66 (0.03), Sadist calls 4 or more in 41% of its casts (1%) and 6 in 30%, burns 0.28 a match worth 3.8 each (0.13 worth 2.1), knockouts 13.2 (12.5), turns per seat 26.6 (26.0). The bots lost their shortfall term with the bill it priced; nothing else in them changed.
...
- `BlindSpot_StrikesAHeavyTargetForThree` (2026-09-23)
- `BlindSpot_PaysNoHeavyBonus_ToACommonTarget` (2026-09-23)
...
- 2026-09-23 — **Blind Spot's strike pays the heavy bonus** (designer): the instant hit deals 3 instead of 2 against a target whose maximum health is above 7, independently of the follow-up, which is unchanged at 1 and 2 (§2.4, §6.5, §10.9). `AbilityEffect` gains `WithHeavyBonus`, a flat per-effect rider added before any crit multiplier and in the same place as the bleeding bonus; it reads the heavy line the multiplier already read, so the two are independent. Bots sweep, 1600 matches, paired seeds: Luka 26% → 27%, Sanity 25% → 24%, everything else flat and all of it inside noise — three of twelve operators are heavy and the cast lands about once a match. **Also corrected: the heavy roster is the Bouncer, Sanity and Revú**, not "the two tanks"; Revú's 8 has cleared the line since the 2026-09-16 +1, and §1.1, §2.4 and §10.9 all said otherwise. `RevuTests.Equilibrium_AMidCostCastHitsHimClean` moved with the change, which is how the error surfaced. The replay fingerprint changed, so replays recorded before this are refused (`RulesFingerprintTests`). Tests 859 → 861.
- 2026-09-24 — **Revú's drain becomes debt** (designer; §3.3, §5.17, §10.11). Leech Round puts the target's seat 2 in debt instead of destroying 2 energy; the debtor's pool pays as its own turn ends, destroyed, and an unpaid remainder draws 1 interest up to a cap of 6 (`EnergyConfig.DebtCap`, `DebtInterest`, both in the replay fingerprint). Sadist deals the target seat's debt, at least 2, splashes half, and clears it. A collision by the debtor's side against a creditor burns the whole debt. Debt is one figure per seat (`PlayerState.Debt`, `Creditors`), never on a piece. `EffectKind` 15 and 16 are renamed and repurposed: `DrainEnergy` → `IncurDebt`, `MissingEnergyDamage` → `DebtDamage`; `EnergyLedger.Drain` is replaced by `IncurDebt`, `CollectDebt` and `WriteOffDebt`; `EndTurnReport` carries a `DebtCollection`. Events: `EnergyDrained` retired; `DebtIncurred`, `DebtCollected`, `DebtCalled` and `DebtBurned` are new. The bots value a loan by the cap's room, Sadist by the debt, charge a debtor for casting into its own bill (`DebtShortfall` 0.6) and credit a collision on a creditor with the debt it burns (`DebtBurn` 0.8). Bots sweep, 800 matches: Revú 25% → 27%, Sadist 1.18 → 0.77 casts; 79% of debt is paid. The replay fingerprint changed (`37363066` → `9914fd33`), so replays recorded before this are refused. Tests 861 → 883 (`DebtTests` 15 new; `RevuTests`, `RevuBotTests`, `RulesTextTests` and `RulesFingerprintTests` moved), 16 of 16 mutations killed.
