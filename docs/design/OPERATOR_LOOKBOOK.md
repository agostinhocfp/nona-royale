...
- **2026-09-21 — LB5b and LB5c passed Play Mode** ("All is well") and are committed. The rig code landed in `521aba2` alongside the audio work, with only this document in `dced581`; both are on the branch, so nothing is missing, only split oddly.

- **2026-09-21 — LB5d, batch 1: Nuetu, Sanity, Syla rigged in three-quarter view.** Chosen first because the two heavies share Bouncer's build and template, and Syla proves the Light build.
  - **Shared:** `DecoMotifs.Limb`, a tapered limb with round caps at both ends. Every new rig's arms and legs are built from it, so no limb reads as a box and no turn opens a seam.
  - **Nuetu, the disc.** One circle of dove-grey with the jacket open on it: the white shirt as a band off-centre toward the facing side, the stepped black chest plate with its fan, a harness strap, the belt and his two coaster discs (the piece shape, on him in every pose). The arms sit inside the circle and the near pauldron is round and rides the upper arm, so the outline stays a disc. Head pass: a round shaved head, heavy jaw, one thickened ear, a small grin, the brass bio-link collar. Cast: the near arm skims forward and low, as if throwing a disc; the tell lights the plate's seam, the harness and the pauldron. Seated: leaning back; his activity is eating, a hand brought to his mouth.
  - **Sanity, the octagon.** The umber apron is one hard-chamfered octagon from the shoulders to the boots, wider than anyone, with the arms inside its outline, the charcoal livery at the shoulders and sleeves, bare forearms, and the brass tool roll across him with two buckles and three tool handles. The prod hangs from his near hand along his leg, so the cast raises it level at the target; its arc is the tell. Head pass: a small heavy shaved head sunk into a thick neck, one brow shadow. Seated: elbows wide on the table; his activity is cleaning the prod, head bent over it.
  - **Syla, the downward triangle, still without the cape.** The drone housings on her shoulders are the widest line; the bodice rides the chest and the skirt rides the hips, so a hit or a lean turns her body without swinging the gown like a plank. The skirt falls to just above the floor and her shoes step under the hem; ivory legs behind it cover any kneel. Head pass: the bob as one black mass cut at the jaw with two carved highlights, a pale face, one shadow, the oxblood mouth, drawn a size up so it holds against the housings. Her own knockout sinks her into the gown rather than kneeling. Cast: the near hand pointed at the target; the housing slits and a hip cradle's slit are the tell. Seated: a drone in her near palm on the table; her activity is turning it over.
  - **Squint at 64 px, rest poses:** Bouncer/Nuetu 0.82, Bouncer/Sanity 0.79, Nuetu/Sanity 0.72, and Syla at 0.56 or under against all three. The first drafts had the three heavies at 0.84–0.89 (the same head-on-a-box-on-legs read); pulling Nuetu's arms and legs inside the circle and widening Sanity's octagon to the boots fixed it. Bouncer/Nuetu is still amber (≥ 0.75), as the front-view pair was.
