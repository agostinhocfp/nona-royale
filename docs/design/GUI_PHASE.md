...
| G4  | **Board texture hookup**                                     | Optional painted marble, felt and carpet loaded by name, with the procedural surfaces as the fallback. Prompts for the textures. (Corner lamps were tried and removed; the table's corners stay dark.)                                                                                                                                                                                                                                                                                                                                                                                 | —                                              |
| H   | **Pause**                                                    | Esc with nothing selected: resume, restart, settings (health labels, log), quit to menu.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| G5  | **Cursor**                                                   | The game's own hardware cursor, drawn from `UiTheme`: a gold Deco arrow everywhere, an amber reticle while an ability waits for a target and the pointer is over one it would take, a grey reticle over anything else on the board.                                                                                                                                                                                                                                                                                                                                                    | —                                              |     | —   |
...
- 2026-09-17 — **G3 passed Play Mode and was committed** (`46c1ce1`, designer: "great stuff"). The phase is closed again; K stays parked.
- 2026-09-17 — **Increment G4 planned and written: corner lamps and the board texture hookup.** It was deferred from G3. The designer's picks (picker, all recommended): LT2 first and G4 after it, as separate commits; **room silhouettes** in the table's corners; **the hookup plus prompts** for painted textures.
  - **Corner lamps** (ART_DIRECTION §6: "layered silhouettes of ornate structures receding into dark").
    - A Deco floor lamp in each corner: a stepped base, a stem with a collar, a two-tier fan shade and a diamond finial. `BoardArt.LampBody` and `LampRim`: 1.7 cells tall, 0.8 of that wide, pivot at the foot.
    - The body is darker than the table (`UiTheme.LampBody`, `#040305`). It reads by a warm lit edge on its board-facing side (`LampRim`, 32%; flipped for the eastern pair), a glow under the shade (`LampGlow`), a painted pool behind it (`LampPool`, 10%) and a contact shadow. There is no gold on it.
    - `SceneLighting` adds a warm multiply light at each lamp's foot (intensity 0.3, reach 1.6 cells; `lampIntensity`, `lampReach`, `lampColour`), off with Lighting effects like the other pools. With effects off, the painted pool and glow keep the corners finished.
    - Geometry is in `BoardLayout`: `TableMargin` moved there from `BoardView`, plus `TableHalf`, `CornerLampCount`, `CornerLampHeight`, `CornerLamp(i)` and `IsWestLamp(i)`. The lamps stand inside the table rule, about a third of a cell clear of the yard tables and inside the title's framing.
    - Board sorting orders −11 to −7 (above the vault, below the haze).
  - **Texture hookup** (`BoardTextures`, `TileSampler`, `Editor/BoardTextureImporter`).
    - Optional files in `Art/Resources/Art/Board/`: `board_marble`, `board_felt` and `board_carpet`. Missing means procedural, as before.
    - Marble is baked into the cross in its own colours (4 cells a tile), with the crack kept; the cross is then tinted `PaintedFloor` (white). Felt is baked as brightness over its own mean (two tiles across, clamped 0.6–1.4), so the seat tint still works. Carpet draws the square table as a tiled sprite (2 cells a tile) tinted `CarpetTint`, replacing the plain table and its grain.
    - `TileSampler` reads the pixels once and samples them with wrapping and bilinear filtering. An unreadable texture is skipped with one warning. `BoardArt`'s marble is split into `Marble` and `CrackShade`, and the cross cache key includes whether a painted marble is in use.
    - The importer (first import only) sets: Default texture, sRGB, no alpha, Read/Write on, uncompressed, mipmaps, bilinear, Repeat, max 1024.
    - Docs: `ART_PIPELINE.md` §5 and §9 (board textures). Prompts are in the project's `ART_PROMPTS.md` (board textures).
  - **Checked:** the view and both editor importers compile against the 6000.6 DLLs. No core changes; 731 tests unchanged. The lamps were previewed in numpy and enlarged once from that preview; the first pass was too spindly to read. The texture path can't run outside Unity.
  - **Play Mode checklist:**
    - A dark lamp stands in each corner, with its lit edge toward the board, a glow under the shade and a warm pool. None touches a yard table, the table rule or the frame, on the title or in a match.
    - With Lighting effects off, the corners still read (painted pool and glow).
    - Without any texture in `Art/Board/`, the board looks exactly as G3 left it, apart from the lamps.
    - With a test PNG named `board_marble` (any seamless image): the cross shows it, tiled every 4 cells, with no visible seams; the edge, sunburst and crack sit over it. Likewise `board_felt` (grain on the seat colour) and `board_carpet` (the table, dimmed). A file imported before the importer compiled shows one `[BoardTextures]` warning and stays procedural until Read/Write is turned on.
    - The first deal doesn't hitch more than it did with G3.
- 2026-09-17 — **G4's corner lamps removed** before commit (designer: "not a fan of the lamps, we can remove them for now"). Gone: `BoardArt.LampBody`, `LampRim` and the lamp builders; `BoardView.DrawLamps` and its orders; `BoardLayout`'s lamp geometry; the `UiTheme` lamp tokens; `SceneLighting`'s lamp lights and fields (that file is back to G3). `BoardLayout.TableMargin` stays, now shared by `BoardView`. G4 is the texture hookup alone, and the table's corners stay dark. The lamp notes above are kept as a record of what was tried. The Play Mode checklist above still applies, minus its two lamp lines.
- 2026-09-17 — **G4 committed** inside the LT2 commit, `9514e90` (`feat(render): LT2 event light…`), together with `Art/Resources/Art/Board.meta` for the empty textures folder. No painted textures exist yet; the board is procedural. The phase is closed again.
- 2026-09-17 — **G5 written: the cursor.** Decided with the designer: its own increment, now, drawn in code rather than imported, with the aim reticle in amber to match the target rings.
  - **New `View/CursorSkin.cs`** (plain C#, owned by `MatchBootstrap`).
    - Three 32×32 looks drawn once at Start: `Arrow` (a tall two-tone gold dart with the ink outline, hotspot at the tip), `Aim` (four inward amber darts round a hollow diamond, hotspot at the centre) and `AimBlocked` (the same darts in `TextDim`, no diamond, 85% opacity).
    - Shapes are exact signed-distance polygons supersampled 4×4, with the outline drawn inside so the silhouettes keep sharp corners (ART_DIRECTION §8).
    - A hardware cursor (`CursorMode.Auto`): the OS draws it, so it never trails the mouse. Textures are RGBA32, readable, without mipmaps, and `alphaIsTransparency` in the editor, which Unity requires of a cursor.
    - `Show` does nothing unless the look changed. `Invalidate` forces the next one through; `Release` restores the system cursor and frees the textures.
    - Not scaled for DPI. If 32 px reads small on a high-DPI screen, a 48 px set chosen by `Screen.dpi` goes in `CursorSkin` only.
  - **`MatchBootstrap`:**
    - Builds the skin in `Start` before anything else is shown; releases it in `OnDestroy`; invalidates it in `OnApplicationFocus`.
    - Any full-screen card (title, setup, draft, pause, results) and the empty room show the arrow.
    - In a match, `PointerCursor` asks what `AimAt` would do with a click, with the same radii and the same engine answers (`_legalCells`, `_castTargets`): the reticle only while an operator and an ability that takes a target or a cell are selected. Over the HUD, over one of the seat's own pieces that isn't a target, on a CPU turn, and after the match, it is the arrow. While the board is busy the reticle shows blocked, because the click is dropped.
    - Set last in `Update`, so it reflects this frame's clicks and keys.
  - **Not touched:** `PauseMenu` (the root's `ModalOpen` already covers it). No core change.
  - **Checked:** the three looks were previewed at 6× and at actual size on the board void, the cross floor and a gold surface; the first pass had too little gold in the arrow and the ticks and was widened. **Not compile-checked:** this chat had no Unity DLLs.
  - **Play Mode checklist:**
    - The title, setup, draft, pause and end screens show the gold arrow, with the tip exactly where clicks land.
    - Selecting an operator and a target ability: amber reticle over an amber-ringed piece, grey over empty board and over enemies out of reach, arrow over your own non-target pieces and over the HUD.
    - A cell ability: amber reticle within snapping distance of a highlighted cell, grey elsewhere on the board.
    - A self-cast ability (no aim) keeps the arrow.
    - Esc and right-click back out of the aim, and the arrow returns in the same frame.
