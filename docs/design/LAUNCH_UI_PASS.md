...
> Status: **Open, 2026-09-26.** G6a–G6e and G7a passed Play Mode and are committed (G7a `049edcd`). **G7b proposed, waiting on a go. G8 (All In 1 Sprite Shader) opened: the pack is imported and trimmed, the integration plan is below; no view code yet.** Open: G7 captures still owed (settings pages, glossary tab, trait and keyword cards, history hover card, doubles callout).
