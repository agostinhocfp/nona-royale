...
> Status: **Open, 2026-09-20.** Opened the day the visual pass closed. H1 is in; H2, H3 and H4 remain.
...
| H2 | **The rail** | Your seat in full rows; each CPU one line with colour, name, home count and three health pips, expanding while it plays. 290 down to about 210. |
| H3 | **A quiet board** | `PieceHudLayer` shows health on hover, on selection and on damage only. Status chips leave the rail and live on the board alone, at a saturation the palette can hold. |
