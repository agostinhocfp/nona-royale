...
- **In game:** `bouncer_standing.png`, `bouncer_seated.png` and `bouncer_portrait.png` copied into `Assets/_Project/Art/Resources/Art/Operators/` (version C). Unity writes the `.meta` files and `OperatorArtImporter` applies the settings on first import. Play Mode check pending: his height and head against Luka at board zoom, the seated bust at the table.
- Re-render command: `--model art\source\characters\bouncer\bouncer_rig.blend --name bouncer --head 1.7 --legs 0.83 --arms 1`.
